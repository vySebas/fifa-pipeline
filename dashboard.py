...
def actualizar_dashboard(pais_seleccionado):
    ...
    paises = [row[0] for row in conn.execute(
        "SELECT DISTINCT nationality FROM main.dim_teams WHERE nationality IS NOT NULL ORDER BY nationality"
    ).fetchall()]
    opciones_dropdown = [{'label': p, 'value': p} for p in paises]

    if not pais_seleccionado:
        df_goles = conn.execute("""
            SELECT dt.nationality, SUM(f.goals) as goles
            FROM main.fct_player_performance f
            JOIN main.dim_teams dt ON f.nationality = dt.nationality
            GROUP BY dt.nationality ORDER BY goles DESC LIMIT 10
        """).df()
        df_pos = conn.execute("""
            SELECT dp.position, COUNT(f.player_id) as total
            FROM main.fct_player_performance f
            JOIN main.dim_positions dp ON f.position = dp.position
            GROUP BY dp.position
        """).df()
        df_disp = conn.execute("""
            SELECT dt.nationality, SUM(f.expected_goals_xg) as xg, SUM(f.goals) as goles
            FROM main.fct_player_performance f
            JOIN main.dim_teams dt ON f.nationality = dt.nationality
        ...
        df_goles = conn.execute("""
            SELECT dpl.player_name, f.goals
            FROM main.fct_player_performance f
            JOIN main.dim_players dpl ON f.player_id = dpl.player_id
            WHERE f.nationality = ? ORDER BY f.goals DESC LIMIT 10
        """, [pais_seleccionado]).df()
        df_pos = conn.execute("""
            SELECT dp.position, COUNT(f.player_id) as total
            FROM main.fct_player_performance f
            JOIN main.dim_positions dp ON f.position = dp.position
            WHERE f.nationality = ? GROUP BY dp.position
        """, [pais_seleccionado]).df()
        df_disp = conn.execute("""
            SELECT dpl.player_name, f.expected_goals_xg as xg, f.goals, f.position
            FROM main.fct_player_performance f
            JOIN main.dim_players dpl ON f.player_id = dpl.player_id
            WHERE f.nationality = ?
        """, [pais_seleccionado]).df()
